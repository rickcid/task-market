...
app.factory('Auth', function(FURL, $firebaseAuth, $firebase) {//defines functionality of service
  ...
  var auth = $firebaseAuth(ref);//drafts the authentication methods provided by firebase library
  //to authenticate users responding to change in auth state and changes in email or passw
  //passes ref to Firebase to firebaseAuth & assigns it to the var auth

  var Auth = {//Methods with $ provided by firebase 

    user: {},//empty object that stores user data and status, that will be used in controller
    ...
    register: function(user) {
      return auth.$createUser({email: user.email, password: user.password})
        .then(function() {
          return Auth.login(user);//after registering, logs in
        })
        .then(function(data) {
  ...
  auth.$onAuth(function(authData) {//function will allow us to know when user logs in/out, in order to release resource. $onAuth listens to auth state
    if(authData) {                //fires when authentication state changes
      angular.copy(authData, Auth.user);//when user logs in we copy the user data to Auth.user
      ...
      angular.copy({}, Auth.user);//when user logs out we release the resource
    }
  });

  //Gravatar
  function get_gravatar(email, size) {//sends email and size wanted and returns grav link
      email = email.toLowerCase();

      var MD5 = function(s) {//MD5 Hash generator for email
        function L(k, d) {
          return (k << d) | (k >>> (32 - d))
        }
